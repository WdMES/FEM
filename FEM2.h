...
double FEM<dim>::basis_function(unsigned int node, double xi){
  ...
  double nom=1.;
  double denom=1.;
  for (int B = 1; B <= basisFunctionOrder; B++){
    if(B!=node){
      nom*=(xi-pow(xi,B));
      denom*=(pow(xi,node)-pow(xi,B));
    }
  }
  value = nom/denom;
  ...
  for (;elem!=endc; ++elem){
    ...
    for(unsigned int A=0; A<dofs_per_elem; A++){
      for(unsigned int q=0; q<quadRule; q++){
        ...
        x*=DEF_f*DEF_A*h_e*.5*basis_function(A, quad_points[q]); 
        Flocal[A]=x;
      }
    }
	...
	//EDITED - Modify Flocal to include the traction on the right boundary.
        Fi_temp = x+DEF_A*DEF_h;
        Flocal[dofs_per_elem-1]=Fi_temp;
      }
    }
    ...
    for(unsigned int A=0; A<dofs_per_elem; A++){
      for(unsigned int B=0; B<dofs_per_elem; B++){
          ...
          Ki_temp += basis_gradient(A, xi_at_node(q))*basis_gradient(B, xi_at_node(q))*quad_weight[q];
    
	      }
        Ki_temp=0;
        Klocal.add(A,B,Ki_temp);
      }
    }
