...
#define DEF_L 3.51e-7
#define DEF_g1 0
#define DEF_g2 0.3362269
#define DEF_A 10e-4
//#define DEF_f 10e11
#define DEF_h 10e6
#define DEF_x_n 8.766782e-8
#define DEF_x_mid 1.75e-7
#define DEF_x_p 2.6278e-7
#define DEF_dens 9.33116e19
...
class FEM
{
 public:
  ...
  //"f" function
  double f_function(unsigned int x);
  ...
  std::vector<double> generate_xi_values();
  double choose_gradient();
  ...
  double                basisFunctionOrder, prob, L, g1, g2, surf, force_amp, E, h;    
...
//Calculating f_function
template <int dim>
double FEM<dim>::f_function(unsigned int x) {
    double f;
    double charge_n = DEF_dens * (DEF_x_mid - DEF_x_n);
    double charge_p = -charge_n;

    if (x < DEF_x_n) {
        f = 0.;
    }
    else if (x > DEF_x_n && x < DEF_x_mid) {
        f = DEF_dens;
    }
    else if (x > DEF_x_mid && x < DEF_x_p) {
        f = charge_p / (DEF_x_p - DEF_x_mid);       //zasada zachowania ladunku, po prawej i po lewej musi byc taki sam, z dokladnoscia do minusa
    }
    else {
        f = 0.;
    }
...
template <int dim>
std::vector<double> FEM<dim>::generate_xi_values()
{
  std::vector<double> Xi_Values;
  for(int i=0;i< (int)(basisFunctionOrder+1);i++)
  {
    Xi_Values.push_back(xi_at_node(i));
  }
  return Xi_Values;
}
...
double FEM<dim>::basis_function(unsigned int node, double xi){
  ...
  double xi_j  = xi_at_node(node);
  xi_values    = generate_xi_values();
  xi_values.erase(xi_values.begin()+node);

  for(int i=0; i<xi_values.size(); i++)
  {
    value *= (xi-xi_values[i])/(xi_j-xi_values[i]);
  }
...
double FEM<dim>::basis_gradient(unsigned int node, double xi){
  ...
  else if(basisFunctionOrder==3)
  {
    for(int i=0; i<xi_values.size(); i++)
    {
      value +=   (xi  -xi_values[i])*(xi  -xi_values[(i+1)%((int)basisFunctionOrder)]);
      some_const     *= 1/(xi_j-xi_values[i]);
    }
...
void FEM<dim>::setup_system(){
  ...
  // quad_points.resize(quadRule); quad_weight.resize(quadRule);
      ...
      quad_points[0] = -sqrt(1. / 3.); //EDITED
      quad_points[1] = sqrt(1. / 3.); //EDITED

      quad_weight[0] = 1.; //EDITED
      quad_weight[1] = 1.; //EDITED

  }
  else if (quadRule == 3) {

      quad_points[0] = sqrt(3. / 5.); //EDITED
      quad_points[1] = 0; //EDITED
      quad_points[2] = sqrt(3. / 5.); //EDITED


      quad_weight[0] = 5. / 9.; //EDITED
      quad_weight[1] = 8. / 9.; //EDITED
      quad_weight[2] = 5. / 9.; //EDITED

  }
  else if (quadRule == 4) {

      quad_points[0] = sqrt(3. / 7. - 2. / 7. * sqrt(6. / 5.)); //EDITED
      quad_points[1] = -sqrt(3. / 7. - 2. / 7. * sqrt(6. / 5.)); //EDITED
      quad_points[2] = sqrt(3. / 7. + 2. / 7. * sqrt(6. / 5.)); //EDITED
      quad_points[3] = -sqrt(3. / 7. + 2. / 7. * sqrt(6. / 5.)); //EDITED

      quad_weight[0] = (18 + sqrt(30.) / 36.); //EDITED
      quad_weight[1] = (18 + sqrt(30.) / 36.); //EDITED
      quad_weight[2] = (18 - sqrt(30.) / 36.); //EDITED
      quad_weight[3] = (18 - sqrt(30.) / 36.); //EDITED
...
//function for evaluating forcing function at x
template <int dim>
...
void FEM<dim>::assemble_system(){
  ...
  for (;elem!=endc; ++elem){
    ...
    for(unsigned int A=0; A<dofs_per_elem; A++)
    {
      for(unsigned int q=0; q<quadRule; q++)
      {
      	...
      	for(unsigned int B=0; B<dofs_per_elem; B++)
        {
      	...
      	Flocal[A]+=basis_function(A, quad_points[q]) * f_function(x) * quad_weight[q];            //*forcing_function(x)*quad_weight[q];    //forcing_function =? f_function

        //EDIT - Define Flocal.
      }
      //std::cout << "globalny index   " << local_dof_indices[A] << "   wartosc calki  " << Flocal[A]<< std::endl;
      Flocal[A]*=surf*h_e/2;
    }
    ...
    for(unsigned int A=0; A<dofs_per_elem; A++)
    {
      ...
      //std::cout<<"writting to F[" << local_dof_indices[A] << "]  the value  " <<  Flocal[A] << std::endl;
      ...
      for(unsigned int B=0; B<dofs_per_elem; B++)
      {
        ...
        //std::cout << "global index   " << local_dof_indices[A] << "," << local_dof_indices[B] << "   wartosc calki  " << Klocal[A][B]<< std::endl;
...
double FEM<dim>::exact_solution(double x)
{
  if((int)prob==1)
  {
    return ((-f_function(x) /(6*E))*pow(x,3)+( (g2-g1)/L + (1./(6*E))* f_function(x) *pow(L,2) )*x + g1);
  }
  else if((int)prob==2)
  {
    return ((-f_function(x) /(6*E))*pow(x,3)+(h/(surf*E)+ f_function(x) *pow(L,2)/(2*E))*x + g1);
  }
  else
  {
    std::cout<<"Error: Unknown problem" << std::endl;
    return 0;
  }
}
...
double FEM<dim>::l2norm_of_error(){
  ...
  for (;elem!=endc; ++elem)
  {
    ...
    for(unsigned int q=0; q<quadRule; q++)
    {
      ...
      l2norm += pow((exact_solution(x)-u_h),2)*h_e*quad_weight[q]/2.;
      //std::cout<< "exact solution - numerical solution:   " << exact_solution(x)-u_h << std::endl;
